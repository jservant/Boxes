...
public class TileController : MonoBehaviour
{
    ...
    private void LateUpdate() // LateUpdate because you want these to process after the player moves
    {
        ...
        bool pMove; // Put this in game controller <<<<<<<<<<<
        if (!gameC.Lose && !gameC.Win) { pMove = GameObject.FindWithTag("Player").GetComponent<PlayerController>().canMove; } else { pMove = false; } // This is mostly to prevent errors
        
        // Have tile activate when player moves, if they can
        if (pMove && !gameC.Win && Input.GetKeyDown(KeyCode.A) || Input.GetKeyDown(KeyCode.LeftArrow)
